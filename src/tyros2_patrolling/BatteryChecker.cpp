...
#include "tyros2_patrolling/BatteryChecker.hpp"
...
BatteryChecker::BatteryChecker(const std::string & xml_tag_name, const BT::NodeConfiguration & conf)
...
void BatteryChecker::update_battery()
{
  ...
  float vel = sqrt(
    last_twist_.linear.x * last_twist_.linear.x + last_twist_.angular.z * last_twist_.angular.z);
...
{
  factory.registerNodeType<tyros2_patrolling::BatteryChecker>("BatteryChecker");
}
